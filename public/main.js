...
(function () {
  ...
  function initCounters() {
    ...
    for (var i = 0; i < counterElements.length; i++) counterObserver.observe(counterElements[i]);
  ...
  function initActiveNavOnScroll() {
    ...
    var onScroll = throttle(function () {
      ...
      for (var i = 0; i < sections.length; i++) {
        ...
        if (window.scrollY >= top) current = sections[i].getAttribute('id');
      }
      for (var i = 0; i < navLinks.length; i++) {
        navLinks[i].classList.remove('active-link');
        if (navLinks[i].getAttribute('href') === '#' + current) navLinks[i].classList.add('active-link');
      }
    }, 100);
    window.addEventListener('scroll', onScroll);
  }
